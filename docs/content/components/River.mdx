...
While a composable API is provided, the rendered layout is fixed and deterministic.

Custom content <strong>cannot</strong> be passed as `children`. Instead use `leadingComponent` and `trailingComponent` props to render app-specific content before or after the main content block.

<Note variant="warning">
  This API should only be used for corner-cases. Any custom content should
  always follow Primer accessibility guidelines where possible.
</Note>

```javascript live noinline
const Example = () => {
  const LeadingComponent = () => (
    <Label size="large" variant="sponsors" sx={{mb: 2}}>
      Leading content
    </Label>
  )
