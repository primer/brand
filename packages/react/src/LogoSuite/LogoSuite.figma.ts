...
const headingText = figma.selectedInstance.getBoolean('heading?', {
  true: figma.selectedInstance.findText('Heading').__render__(),
  false: 'Heading',
})
const headingOpeningTag = figma.selectedInstance.getBoolean('heading?', {
  true: '<LogoSuite.Heading>',
  false: '<LogoSuite.Heading visuallyHidden>',
})
const headingClosingTag = figma.selectedInstance.getBoolean('heading?', {
  true: '</LogoSuite.Heading>',
  false: '</LogoSuite.Heading>',
})
const descriptionText = figma.selectedInstance.getBoolean('description?', {
  true: figma.selectedInstance.findText('Description').__render__(),
  false: undefined,
})
...
const hasDivider = figma.selectedInstance.getBoolean('hasDivider?', {
  true: undefined,
  false: false,
})
...
const logobarProps = (function () {
  const nestedLayer0 = figma.selectedInstance.findInstance('Logobar')
  return {
    variant:
      nestedLayer0.type !== 'ERROR'
        ? nestedLayer0.getEnum('variant', {
            muted: 'muted',
            emphasis: 'emphasis',
          })
        : undefined,
    marquee: nestedLayer0.type !== 'ERROR' ? nestedLayer0.getBoolean('marquee?') : undefined,
  }
})()
...
export default {
  ...
  example: figma.code`<LogoSuite${figma.helpers.react.renderProp('align', align)}${figma.helpers.react.renderProp(
    ...
    hasDivider,
        ...
        ${figma.helpers.react.renderChildren(headingOpeningTag)}
        ${figma.helpers.react.renderChildren(headingText)}
        ${figma.helpers.react.renderChildren(headingClosingTag)}
        ${figma.helpers.react.renderChildren(descriptionOpeningTag)}
        ${figma.helpers.react.renderChildren(descriptionText)}
        ${figma.helpers.react.renderChildren(descriptionClosingTag)}
        <LogoSuite.Logobar${figma.helpers.react.renderProp(
          'variant',
          logobarProps.variant,
        )}${figma.helpers.react.renderProp('marquee', logobarProps.marquee)}>
          ...
          <svg role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 291 79.46" width="291" height="79.46">
            ...
            <path d="M239.33 95.036c0 2.96 2.46 5.361 5.495 5.361h63.376c3.035 0 5.496-2.401 5.496-5.361V30.919c0-2.961-2.46-5.361-5.496-5.361h-63.376c-3.035 0-5.495 2.4-5.495 5.361v64.117z" style={{
        fill: 'inherit',
        fillOpacity: 1,
        fillRule: 'nonzero',
        stroke: 'none',
