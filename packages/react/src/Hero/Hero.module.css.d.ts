declare const styles: {
  readonly "Hero": string;
  readonly "Hero--align-center": string;
  readonly "Hero--align-start": string;
  readonly "Hero--image-pos-inline": string;
  readonly "Hero--variant-default": string;
  readonly "Hero--variant-gridline": string;
  readonly "Hero--variant-gridline-expressive": string;
  readonly "Hero--variant-inline-gridline": string;
  readonly "Hero-actions": string;
  readonly "Hero-content": string;
  readonly "Hero-contentColumn--bordered-inline": string;
  readonly "Hero-description": string;
  readonly "Hero-expressive-body-column": string;
  readonly "Hero-expressive-header-column": string;
  readonly "Hero-eyebrow": string;
  readonly "Hero-grid": string;
  readonly "Hero-grid--block-end": string;
  readonly "Hero-grid--bordered-inline": string;
  readonly "Hero-grid--contained": string;
  readonly "Hero-grid--expressive": string;
  readonly "Hero-grid--inline-end": string;
  readonly "Hero-heading": string;
  readonly "Hero-heading--fullWidth": string;
  readonly "Hero-image": string;
  readonly "Hero-imageContainer--bg-subtle": string;
  readonly "Hero-imageContainer--inline-bordered": string;
  readonly "Hero-imageContainer--inline-padding-all": string;
  readonly "Hero-imageWrapper": string;
  readonly "Hero-imageWrapper--bg-subtle": string;
  readonly "Hero-imageWrapper-inner": string;
  readonly "Hero-imageWrapper-inner--padding-all": string;
  readonly "Hero-imageWrapper-inner--padding-none": string;
  readonly "Hero-imageWrapper-inner--with-gridline": string;
  readonly "Hero-label": string;
  readonly "Hero-media": string;
  readonly "Hero-media--padding-all": string;
  readonly "Hero-media--padding-none": string;
  readonly "Hero-media--pos-block-end": string;
  readonly "Hero-media--pos-inline": string;
  readonly "Hero-media-item": string;
  readonly "Hero-media-item--block-end": string;
  readonly "Hero-media-item--inline": string;
  readonly "Hero-mediaContainer": string;
  readonly "Hero-text-column--inline-gridline": string;
  readonly "Hero-trailing": string;
  readonly "Hero-video": string;
  readonly "Hero-video-header-poster-icon": string;
  readonly "Hero-video-poster-button": string;
  readonly "Hero-video-poster-header": string;
  readonly "Hero-video-poster-icon": string;
  readonly "Hero-video-poster-image": string;
  readonly "Hero-video-poster-text": string;
};
export = styles;

