...
- **Do** — use `InlineCode` for short code fragments and code blocks for multiline examples. Keep the default wrapping behavior unless rendering a slash command that must stay together, then use `wrap={false}`.
