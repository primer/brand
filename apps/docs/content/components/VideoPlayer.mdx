...
The `VideoPlayer` component exposes a `useVideo` hook that can be used to control the video programmatically. To use the hook, the `VideoPlayer` component must be wrapped in a `VideoPlayer.Provider` component, which usually is done automatically for convenience.

If you want to take advantage of the `useVideo` hook, you should use the `VideoPlayer.WithoutProvider` component instead of the `VideoPlayer` component and manually wrap it in a `VideoPlayer.Provider` component.
  ...
  const MyVideoPlayer = () => {
    ...
    return (
      <Stack direction="vertical">
        <VideoPlayer.WithoutProvider title="GitHub media player"
          showPlayPauseButton={false}
