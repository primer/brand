...
Use the `Stack` or `Grid` component to stack two cards horizontally. Note that in this arrangement, both `fullWidth` and `padding` options should be uniquely applied.
...
<Stack
  ...
  gap="normal"
>
  <Card href="https://github.com" hasBorder padding="normal" fullWidth>
...
| `padding`       | <PropTableValues values={CardPaddingSizes} addLineBreaks /> | `condensed` | `false`  | The internal spacing separating outer and inner card content |
