...
The call to action text defaults to `Learn more` and can be customized using the `ctaText` component.
...
<Note>
  Note that in this arrangement, both <InlineCode>fullWidth</InlineCode> and{' '}
  <InlineCode>hasBorder</InlineCode> options should be applied alongside{' '}
  <InlineCode>align="center"</InlineCode>.
</Note>
