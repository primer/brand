...
When using the `SubdomainNavBar`, use the `onMobileMenuToggle` prop to detect when the mobile menu is opened or closed.

When the menu is open, ensure that the rest of the document is hidden from screen readers. This can be achieved by adding `aria-hidden="true"` or `inert` to the main content area when the menu is open.
...
| `onMobileMenuToggle` | `(isOpen: boolean) => void`                                                                                                                                                            |                     | When the mobile menu is opened or closed, this callback is called with the new open state.                                             |
