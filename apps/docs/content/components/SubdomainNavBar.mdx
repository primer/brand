...
When using the `SubdomainNavBar`, use the `onNarrowMenuToggle` prop to detect when the menu is opened or closed on narrow viewports.
...
| `onNarrowMenuToggle` | `(isOpen: boolean) => void`                                                                                                                                                            |                     | When the menu is opened or closed on narrow viewports, this callback is called with the new open state.                                |
