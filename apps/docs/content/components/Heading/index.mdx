...
The Heading scale ranges from 1-6 and maps to the HTML heading elements h1-h6 respectively. The size is determined by the type of HTML element specified, although you can override this when the scenario calls for it. E.g. use a `h4` heading size on a `<h3>` to highlight an article that might be relevant to the user.
