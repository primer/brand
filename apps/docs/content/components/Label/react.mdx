...
Use the `size` prop to change the size of the `Label` component. The `medium` size is recommended for small titles or headings, the `large` size is recommended for large titles or section intros, and the `small` size is recommended for body size texts.
