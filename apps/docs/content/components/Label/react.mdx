...
Use the `size` prop to adjust the size of the `Label` component. The `medium` size is recommended for small titles or headings, the `large` size is suitable for large titles or section intros, and the `small` size is ideal for body text.
