...
Available options include `start`, `center`, and `end`, which are recommended for supporting bidirectional reading. In situations where fixed alignment is needed irresepctive of reading direction, you can also utilize the `left` and `right` options.
