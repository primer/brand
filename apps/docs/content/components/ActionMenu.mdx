...
| `className` | `string`       |         |          | Sets a custom class on the element                          |
| `children`  | `ReactElement` |         |          |
| `disabled`  | `boolean`      | `false` | `false`  | Allows control over an indivual items active/inactive state |
| `id`        | `string`       |         |          | Sets a custom `id`                                          |
| `selected`  | `boolean`      |         |          | Indicates the item is selected n `single` selection mode    |
