...
Derived values such as `dark_dimmed` and `light_high_contrast` resolve internally to `dark` or `light`, which is also the value returned by `useTheme().colorMode` and applied to `data-color-mode`.
