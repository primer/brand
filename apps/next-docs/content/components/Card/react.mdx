...
import {Avatar, Box, Grid, Token} from '@primer/react-brand'
import {
  CardVariantsProp,
  CardCTAVariantsProp,
...
import {MicrosoftLogo} from '@primer/react-brand/fixtures/third-party-logos/MicrosoftLogo'
...
Use `leadingVisual` for a logo or brand mark that should sit above the card content. Do not combine `leadingVisual` with `Card.Icon`; use one or the other depending on the design.

```jsx live
<Card href="https://github.com" ctaVariant="arrow" leadingVisual={<MicrosoftLogo />}>
...
Use `Card.Label` for short eyebrow copy above the heading. `Card.Label` is built on `EyebrowText` and supports `variant="default"`, `variant="muted"`, and `variant="accent"`; use `accent` when the design calls for the green emphasis treatment.
...
Use `Card.Tokens position="block-end"` when tokens should sit below the description. The heading and description still render before the tokens in the DOM, so the reading order stays aligned with the visual order.

```jsx live
<Card href="https://github.com" hasBorder fullWidth ctaVariant="none">
...
| name       | type                       | default  | required | description                                |
| ---------- | -------------------------- | -------- | -------- | ------------------------------------------ |
| `children` | `ReactNode`, `ReactNode[]` |          | `true`   | Eyebrow content rendered above the heading |
| `variant`  | <CardLabelVariantsProp />  | `accent` | `false`  | Visual variant of the label copy           |

Forwards all the props from the [EyebrowText component](/components/EyebrowText), including `className`, `id`, and `ref`.
