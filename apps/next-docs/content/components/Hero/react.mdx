...
Use `variant="bordered-grid"` to display the Hero with a contained image, subtle background color, and borders. This is a convenient single-prop alternative to manually combining `imageBackgroundColor="subtle"` with `Hero.Image fullWidth={false}`.
