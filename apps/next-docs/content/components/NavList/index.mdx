...
Add `NavList.SubNav` to a `NavList.Item` to turn it into a section. Section rows render as disclosure buttons that expand and collapse their nested items.
...
### Maximum limit of five nested sub navigations
...
Give each `NavList` a clear `aria-label` or `aria-labelledby`, especially when the page includes more than one navigation landmark.

Use `aria-current="page"` on the link for the current page. Only apply `aria-current` to leaf items that navigate somewhere, not to section rows with nested items.

Keep section labels short and descriptive. Section rows with `NavList.SubNav` must include visible label text so people can understand what will expand.
...
| name               | type                       | default | required | description                                                                             |
| ------------------ | -------------------------- | ------- | -------- | --------------------------------------------------------------------------------------- |
| `as`               | `React.ElementType`        | `'a'`   | `false`  | Element or component used for leaf items                                                |
...
| `disabled`         | `boolean`                  | `false` | `false`  | Disables the item. Leaf links receive `aria-disabled`; section buttons receive disabled |

Forwards props to the rendered leaf link or section button.
...
Forwards all standard HTML attributes for `<ul>` elements.
