...
The `leadingComponent` and `trailingComponent` props are supported composition slots for custom React elements. On large viewports, `leadingComponent` appears after the navigation links, and `trailingComponent` appears after search and before the actions. On narrow viewports, `leadingComponent` moves before the navigation links inside the menu, while `trailingComponent` appears before the actions in the menu footer.
...
| `leadingComponent`      | `React.ReactElement`                 |                      | Custom element rendered after navigation, or before it in narrow menus  |
| `trailingComponent`     | `React.ReactElement`                 |                      | Custom element rendered after search and before actions                 |
