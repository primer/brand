...
export function SearchNav() {
  ...
  return (
    <>
      ...
      <SubdomainNavBar ref={navRef} title="GitHub Docs" fixed={false} fullWidth variant="gridline">
        <SubdomainNavBar.Link href="/en/get-started">Get started</SubdomainNavBar.Link>
...
Use the `gridline` variant for designs that need horizontal and vertical separators and a full-width accent rule. The exported `SubdomainNavBarVariants` and `SubdomainNavBarVariant` describe the available root variants; `SubdomainNavBarSearchVariants` and `SubdomainNavBarSearchVariant` do the same for search.

```jsx live
<SubdomainNavBar title="GitHub Docs" fixed={false} fullWidth variant="gridline">
  <SubdomainNavBar.Link href="#plans">Free, Pro & Team</SubdomainNavBar.Link>
</SubdomainNavBar>
```
...
| `variant`            | `SubdomainNavBarVariant`           | `default`            | `default` or `gridline`; `gridline` adds separators and a full-width accent rule |
...
`SubdomainNavBarProps`, `SubdomainNavBarHandle`, `SubdomainNavBarVariant`, and `SubdomainNavBarVariants` are exported from `@primer/react-brand`.
...
| `isExternal`  | `boolean` |          | Shows an external-link indicator for grouped results in the `gridline` root variant |
