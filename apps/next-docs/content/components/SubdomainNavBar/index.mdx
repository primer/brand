...
When search is present, users can press `/` to open the search dialog. Set `keyboardShortcut={false}` on `SubdomainNavBar.Search` to turn off the shortcut, or pass another key value such as `keyboardShortcut="Command+Option+k"` to remap it. To open the same dialog programmatically, attach a ref to `SubdomainNavBar` and call `openSearch()`.
...
const App = () => {
  ...
  return (
    <>
      <button type="button" onClick={() => navRef.current?.openSearch()}>
        Open search
      </button>
      <SubdomainNavBar ref={navRef} title="Subdomain" fixed={false}>
...
Use the `gridline` variant for designs that need visible cell dividers and the full-width accent rule.
...
| `variant`            | `'default' \| 'gridline'`       |      `default`      | Applies the visual treatment. Use `gridline` to add separators and the full-width accent rule.                                         |
| `ref`                | `React.RefObject`               |                     | Forward a Ref to the underlying DOM node, with `openSearch()` and `closeSearch()` methods                                              |
