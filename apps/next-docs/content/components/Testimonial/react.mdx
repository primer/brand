...
Use the `layout` prop to change the internal arrangement of the testimonial content. The `wide` layout places the quote on the left and the attribution on the right.
...
| `quoteMarkHasBackground` | `boolean`                         |   `false`   | When true, renders the quote mark inside a colored background box.                                                                  |
...
| `layout`                 | <TestimonialLayoutProp />         | `'default'` | Sets the layout. `'default'` stacks content in a single column. `'wide'` places the quote on the left and attribution on the right. |
